...
def fe():  #aac
    ...
    protein_index1 = 1
    Value=[]
    with open('C:\\Users\\86151\\Desktop\\QSP\\dependent\\AAC.txt', 'r') as t:
        for line in t:
    ...
    # print(protein_seq_dict1)
    label=[]
    # print(Value[39][-1].split('|')[-1])
    for i in range(40):
        label.append(Value[i][-1].split('|')[-1])
    # print(label)
    Label=[]
    for i in range(40):
        if label[i]=='1':
...
def ac():
    ...
    protein_seq_dict = {}
    protein_index = 1
    with open('C:\\Users\\86151\\Desktop\\QSP\\dependent\\AC.txt', 'r') as fp:
        for line in fp:
            if line[0] == '>':
...
def gaac():#gaac
    protein_seq_dict1 = {}
    protein_index1 = 1
    with open('C:\\Users\\86151\\Desktop\\QSP\\dependent\\GAAC.txt', 'r') as t:
        for line in t:
            seq = line[:-1].split('\t')[1:]
            protein_seq_dict1[protein_index1] = seq
            protein_index1= protein_index1 + 1
